...
def test_flat_prices_yield_flat_equity(flat_prices: pd.Series) -> None:
    ...
    result = run_backtest(flat_prices, signals, fill="decision_close", fee_bps=0, slippage_bps=0)
...
def test_mismatched_index_raises() -> None:
    ...
    with pytest.raises(ValueError, match="same index"):
        run_backtest(a, b, fill="decision_close")
...
def test_short_series_raises() -> None:
    ...
    with pytest.raises(ValueError, match="at least 2"):
        run_backtest(prices, signals, fill="decision_close")


def test_metrics_keys_present(rising_prices: pd.Series) -> None:
    signals = pd.Series(1.0, index=rising_prices.index)
    result = run_backtest(rising_prices, signals, fill="decision_close")
    for key in (
        "annualized_return",
        "sharpe",
        "sortino",
...
def test_n_trials_lowers_deflated_sharpe(rising_prices: pd.Series) -> None:
    ...
    one = run_backtest(rising_prices, signals, fill="decision_close", n_trials=1)
