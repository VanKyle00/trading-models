...
full bid-ask spread <= 10% of mid, open interest >= 100 — plus a finite IV > 0, since
...
def _naive(ts: pd.Timestamp) -> pd.Timestamp:
    """Strip tz: chain frames are tz-naive; a tz-aware asof must not silently mismatch."""
    ts = pd.Timestamp(ts)
...
def liquid_quotes(
    ...
    expiration = _naive(expiration)
    asof = _naive(asof)
    sub = chain[(chain["right"] == right) & (chain["expiration"] == expiration)]
    dte = int((expiration - asof).days)
...
def by_delta(quotes: list[LegQuote], target: float) -> LegQuote | None:
    """Quote with delta nearest ``target``. Put deltas are NEGATIVE (BS
    convention) — pass a negative target when picking puts."""
...
def atm_iv(
    ...
    if exp is None:
        return None
    # both rights: the nearest-to-spot CALL can fail the gate while the put passes
